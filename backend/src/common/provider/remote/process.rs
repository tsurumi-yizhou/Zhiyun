...
impl ExecutionProvider for RemoteProcess {
    async fn execute(&self, _command: &str, _options: ExecuteOptions) -> anyhow::Result<ExecuteResult> {
        // Mock: 远程执行逻辑（如通过 SSH）
        Ok(ExecuteResult {
            exit_code: 0,
...
mod tests {
    ...
    async fn test_remote_process_mock() {
        ...
        let result = process.execute("ls", ExecuteOptions::default()).await.unwrap();
